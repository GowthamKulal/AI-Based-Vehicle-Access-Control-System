...
import { DeleteIcon, MoreHorizontalIcon, ShieldBan } from "lucide-react";
...
const MoreAction = ({ visitorId }: { visitorId: string }) => {
  ...
  async function unauthorizeVisitor() {
    ...
    if (response.success) {
      toast.success("Visitor has been un-authorized!");
    } else {
      toast.error("Something went wrong while unauthorizing visitor.");
    }
      ...
      >
        <DropdownMenuGroup>
          <SmartAlertDialog
            ...
            title={"Are you sure?"}
            description={"This visitor will be unathorized."}
            cancelButtonText={"Cancel"}
            confirmButtonText={"Confirm"}
            onConfirmClick={unauthorizeVisitor}
          />
          <DropdownMenuSeparator />
          <SmartAlertDialog
            component={
